...
תיאור מקורי מהמשתמש (החשוב ביותר לשיפוט!): "{state.get('user_input')}"

רגש: {state.get('emotion')}
מחווה: {state.get('gesture')}
מאפיינים שפורקו על ידי שלב קודם: {features}

שימי לב: שלב הפירוק למאפיינים לפעמים **ממציא** פרטים ספציפיים (כמו צבע לחיים
מסוים, שיניים חשופות, קווי הדגשה) גם כשהמשתמש לא ביקש אותם בכלל וכתב תיאור
כללי. אל תשפטי דמיון לפי הפרטים המומצאים האלה - שפטי לפי **התיאור המקורי
שהמשתמש בפועל כתב**. אם התיאור המקורי כללי ובסיסי (כמו "פרצוף מחייך"),
זה כן דומה למועמד בסיסי מתאים, גם אם שלב הפירוק הוסיף פרטים שלא התבקשו.
